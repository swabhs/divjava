...
/**
 * Abstract class for dual decomposition
 * @author sswayamd
 *
 */

public class DualDecompositionHelper<T> {
	
	List<Map<T, Double>> init(int n, TagSet<T> tagSet) {
	...
	/** Is this required? **/
	List<Map<T, Integer>> computeIndicators(List<T> tagSequence, TagSet<T> tagSet) {
	...
	List<Map<T, Double>> update(
			List<Map<T, Double>> dd, List<T> tags1, List<T> tags2, double stepSize, TagSet<T> tagSet) {
	...
	boolean agree(List<T> tags1, List<T> tags2) {
