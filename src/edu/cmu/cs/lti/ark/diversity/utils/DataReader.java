...
public class DataReader {
	...
	public static Map<String, Double> readHmmParams(String weightsFileName) {
		List<String> lines = FileUtils.readFile(weightsFileName);
	...
	public static TagSet<String> readTagset(String tagSetFileName) {
		return (new TagSet<String>(FileUtils.readFile(tagSetFileName)));
	}
	...
	public static Tuple<List<List<String>>, List<List<String>>> readData(String dataFileName) {
		...
		List<String> lines = FileUtils.readFile(dataFileName);
		for (String line : lines) {
			String elements[] = line.split("\t");
	...
	static List<List<String>> readSentences(String sentFileName) {
		List<List<String>> sentences = new ArrayList<List<String>>();
		List<String> lines = FileUtils.readFile(sentFileName);
		for (String line: lines) {
			List<String> sentence = new ArrayList<String>();
			String elements[] = line.split("\t");
	...
	public static List<double[][]> readEdgeWeights(String edgeWeightFileName) {
		...
		List<String> lines = FileUtils.readFile(edgeWeightFileName);
		int i = 0;
		for (String line: lines) {
			if (line.startsWith("##")) {
				if (matrix!= null)
	...
	public static List<List<Integer>> readDepParse(String depFileName) {
		List<List<Integer>> depParses = new ArrayList<List<Integer>>();
		List<Integer> depParse = new ArrayList<Integer>();
		
		List<String> lines = FileUtils.readFile(depFileName);
		for (String line: lines) {
			if (line.equals("")) {
